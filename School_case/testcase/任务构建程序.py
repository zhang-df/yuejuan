...
def 任务构建程序():
    try:
        dr = Browser()
        num = 1  # 循环起始数
        task_name = u'校级任务' + time.strftime('%Y') + '年' + time.strftime('%m') + '月' + time.strftime(
            '%d') + '日' + time.strftime(' %H-%M-%S')
        # 登录进入二一阅卷
        ...
        dr.click_('xpath', '//a[text()="校级"]')
        dr.click_('xpath', '//a[text()="单元考"]')
        dr.click_('xpath', '//a[text()="一年级"]')
        dr.input_('id', 'examtask_Name', task_name)
        dr.click_('xpath', '//a[text()="临时考号"]')
        # dr.click_('xpath', '//a[text()="校级自定义"]')
        sleep(0.5)
        dr.input_('id', 'numberLength', '9')
        dr.click_('xpath', '//div[text()="二维码"]')
        sleep(3)
        dr.click_('css', "[value='确定']")
        sleep(3)
        dr.is_displayed('xpath', '//div[text()="考试任务创建成功"]')
        dr.click_('xpath', '//a[text()="创建科目"]')
        sleep(1)

        # dr.click_('xpath', '//a[text()="返回考试列表"]')
        # while num <= 10:
        #     try:
        ...
        sleep(1)
        # 创建题卡
        boolean = createCard(dr)
        # 复制已有题卡
        # boolean = copyCard(dr)
        dr.is_displayed('xpath', '(//a[contains(@title,"编辑题卡")])[1]')
        sleep(1)
        # 客观题设置
        single = ('A', 'B', 'C', 'D')
        judgment = ('T', 'F')
        ...
        sleep(1)
        # 复制题卡
        # boolean2 = copyCard(dr)
        # 选择组卷题卡
        boolean2 = copyZuJuanCard(dr)
        ...
        dr.click_('xpath', '(//td[text()="一年级(1)班"]/../td)[1]')
        dr.click_('xpath', '(//td[text()="一年级(2)班"]/../td)[1]')
        ...
        dr.driver.switch_to.frame('layui-layer-iframe4')
        dr.is_displayed('xpath', '//a[text()="下一页"]')
        ...
        pyperclip.copy(r'D:\PyCharm\File\Import\intranet\临时考号.xls')
        pyautogui.hotkey('ctrl', 'v')
        pyautogui.press('enter')
        sleep(2)
        dr.click_('xpath', '//button[text()="导入"]')
        dr.is_displayed('xpath', '//div[text()="导入成功，受影响行数：101"]')
        dr.is_displayed('xpath', '(//td[text()="210118080"])[3]')
        dr.driver.switch_to.parent_frame()
        dr.click_('xpath', '//a[text()="确定"]')
        sleep(2)
        print('班级参考完毕')
...
# 复制组卷题卡
def copyZuJuanCard(dr):
    dr.click_('xpath', '//label[@style="margin-right:10px;position: relative;"]')
    sleep(0.5)
    dr.input_('id', 'search-key', '自动化组卷题卡')
    sleep(0.5)
    dr.click_('id', 'btn-search')
    sleep(2)
    dr.is_displayed('xpath', '//input[@value="1997139"]')
...
# 复制题卡
def copyCard(dr):
    dr.click_('xpath', '//input[@value="2"]')
    sleep(0.5)
    dr.click_('xpath', '(//tr[1]/td)[2]/input')
    sleep(0.5)
    dr.click_('id', 'btn-exist-create')
    sleep(1)
    return False
...
# 创建题卡
def createCard(dr):
    dr.input_('id', 'TiKaTitle', '自动化空白题卡' + time.strftime('%m%d'))
    dr.click_('id', 'btn-empty-create')
    num = 1
    while num <= 10:
        try:
            dr.driver.find_element_by_xpath('//p[text()="加载中...."]').is_displayed()
        except Exception as e:
            if num == 10:
                raise e
            else:
