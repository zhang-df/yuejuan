...
def 任务构建程序():
    num = 1  # 循环起始数
    dr = Browser_()
    strf = time.strftime('%Y') + '年' + time.strftime('%m') + \
        '月' + time.strftime('%d') + '日' + time.strftime(' %H-%M-%S')
    task = u'校级任务'
    task_name = task + strf
    ...
    dr.is_displayed('xpath', '//div[text()="导入成功，受影响行数：100"]')
    dr.is_displayed('xpath', '(//td[text()="210118001"])[3]')
    dr.driver.switch_to.parent_frame()
    dr.click_('xpath', '//a[text()="确定"]')
    sleep(2)
    print('班级参考完毕')
